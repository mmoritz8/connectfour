document.addEventListener('DOMContentLoaded', () => {
    ...
    for (var i = 0, len = squares.length; i < len; i++){
        (function(index) {
            // add on click squares for grid
            squares[i].onclick = function(){
                // if the square below the current one is taken, you can go on top of it.
                if (squares[(index + 7)].classList.contains('taken')) {
                    ...
                    // if the square below your currentsquare is not taken, alerts you can't go here. 
